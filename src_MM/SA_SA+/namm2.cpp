...
double computeLowerBound(HyperGraph &hg, unsigned int n, unsigned int k, long long num_edge, double epsilon, double delta, double inf_f, vector<int> &seeds) {
	...
	double weighted_coverage = coverage;
...
int main(int argc, char ** argv)
{
	...
	const double alpha1 = sqrt(log(6.0 / delta));
	const double beta1 = sqrt((1 - 1.0 / e) * (logcnk(n , k) + log(6.0 / delta)));


	double theta_nought = size_t(2.0 * pow2((1 - 1 / e) * alpha1 + beta1));
	...
	const double a1 = log(i_max * 3.0 / delta);
	const double a2 = log(i_max * 3.0 / delta);
	...
	while (true) {
		...
		double cov_r2;
		cov_r2 = computeLowerBound(coll_two, n, k, cur_samples, epsilon_prime, delta_iter, inf_f, seeds);
		lower = pow2(sqrt(cov_r2/2 + a1 * 2.0 / 9.0) - sqrt(a1 / 2.0)) - a1 / 18.0;
		cout << "lower is " << lower << endl;

		upper = pow2(sqrt(cov_ub/2 + a2 / 2.0) + sqrt(a2 / 2.0));
