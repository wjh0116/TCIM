...
public:
	...
	static void CP_based(InfGraph& g, Argument& arg)
	{
		...
		g.init_hyper_graph();
		g.deadline_1=arg.T;
		cout<<"number of CB: "<<CB.size()<<endl;
		...
		initial_ADinf = g.MC_based_estimate_AD_inf(10000, arg.T) - g.rumorSet.size() * g.U(g.deadline_1);
		...
		cout<<"inf computation is done!"<<endl;
		...
		double inf_upper = g.estimate_inf_byStop(g.rumorSet, g.UB_seedSet, 0.25, 1.0 / g.n);
		cout << "upper's influence:" << inf_upper << endl;

		double inf_lower = g.estimate_inf_byStop(g.rumorSet, g.LB_seedSet, 0.25, 1.0 / g.n);
		cout << "lower's influence:" << inf_lower << endl;

		double inf_or = g.estimate_inf_byStop(g.rumorSet, g.Or_seedSet, 0.25, 1.0 / g.n);
		...
		after_blocking_ADinf = g.output_AD_influence(arg) - g.rumorSet.size() * g.U(g.deadline_1);
