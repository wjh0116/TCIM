...
public:
	...
	void WS_TRW_R1(Argument& arg)
	{
		...
		int T=arg.T;
		// Lhyperedge.push_back(make_pair(source_node, U(T-AT[source_node]))); 
		Lhyperedge.push_back(make_pair(source_node, computeU(source_node, AT[source_node]))); // source node
		if(Flag[L_node]==0)
		{
			// cout<<"1"<<endl;
			while(!isRumor[pred[L_node][0]])// need attention
			{
				...
				double weight;
				if(aft_AT==-1)
					weight=computeU(source_node, AT[source_node]);
				else
					weight=computeU(source_node, AT[source_node])- computeU(source_node, aft_AT);
		...
		{
			Uhyperedge.push_back(make_pair(U_node, computeU(source_node, AT[source_node])));
	...
	*/
	...
	void generate_sample_TRtree(vector<vector<int>>& sample,vector<vector<int>>& sample_UB,vector<vector<int>>& sample_delay, double &sum, int deadline, vector<int>& AT)
	{
		priority_queue<Node> Q;
		...
		while (!Q.empty()) 
		{
			...
			for(int i=0;i<gT[q].size();i++)
			{
				...
				if(sfmt_genrand_real1(&sfmtSeed) <= probT[q][i] && AT[q] + diff_delay <= deadline)
				{
					...
					sample_delay[q].push_back(diff_delay);
					if (AT[q] + diff_delay < AT[u]) {
						AT[u] = AT[q] + diff_delay;
						pred[u].clear();
						pred[u].push_back(q);
			...
			I[q] = 1;
			if(!isRumor[q]) sum+=computeU(q, AT[q]);
			if(pred[q].size()!=0)
			{
				int pre=pred[q][0];
	...
	double rep_approximation(Argument& arg){
		...
		for(int i=0;i<10000;i++){
			...
			vector<vector<int>> sample_delay;
			sample_delay.resize(n);

			vector<vector<int>> sample_UB;
			sample_UB.resize(n);
			vector<int> AT(n, 10000); 
			generate_sample_TRtree(sample, sample_UB,sample_delay, ori_ulitity, arg.T, AT);
			// cout<<"ori_ulitity: "<<ori_ulitity<<endl;
			double decrease_upper=0, decrease_ori=0;

			queue<int> q;
			vector<int> vis2;
			vis2.resize(n, 0);
			for (auto x : UB_seedSet)
				q.push(x), vis2[x]=1, decrease_upper+=computeU(x,AT[x]);
			while (!q.empty())
			{
				int x = q.front();
				q.pop();
				for(int v:sample_UB[x]){
					if(vis2[v]==0){
						// cout<<computeU(v,AT[v])<<endl;
						q.push(v);
						decrease_upper+=computeU(v,AT[v]);
						vis2[v]=1;
					}
				}
			...
			while (!Q.empty()) 
			{
				...
				for(int i=0;i<sample[q].size();i++)
				{
					int u=sample[q][i];
					if(isUB[u]==1) continue;
					int diff_delay=sample_delay[q][i];

					
					if(AT[q] + diff_delay <= deadline_1)
					{
						if (AT[q] + diff_delay < AT[u]) {
							AT[u] = AT[q] + diff_delay;
							Q.push({u, AT[u]});
				...
				// cout<<"11"<<endl;
				I[q] = 1;
				if(!isRumor[q]) aft_ulitity+=computeU(q, AT[q]);

			}
			// cout<<"aft_ulitity: "<<aft_ulitity<<endl;
			//----
			// queue<int> q;
			...
			// 	int x = q.front();
			// 	q.pop();
			// 	if(!isRumor[x]) aft_ulitity+=computeU(x,AT[x]);

			// 	for(int v:sample[x]){
			// 		if(isUB[v]==1 || vis3[v]==1)continue;
					
			// 		q.push(v);
			// 		// if(!isRumor[v]) aft_ulitity+=computeU(v,AT[v]);
			...
			// cout<<"decrease_ori: "<<decrease_ori<<", decrease_upper: "<<decrease_upper<<endl;
			// const double eps = 1e-8;
			// if(decrease_ori>decrease_upper + eps){
	...
	void SandTCIM(Argument& arg)
	{
		...
		int T=arg.T;
		int X_max=1; // based on U setting
		const double e = exp(1);
	...
	void generateStratifiedVotingTimes(double T) {
		static thread_local std::mt19937 gen(std::random_device{}());
		std::uniform_real_distribution<> early_voters(0.0, 0.3 * T);
		std::uniform_real_distribution<> late_voters(0.3 * T, T);
		std::uniform_real_distribution<> selector(0.0, 1.0);
		
		for (int v = 0; v < n; ++v) {
			double rand_val = selector(gen);
			if (rand_val < 0.6) {
				// 60% 的节点从 [0, 0.3T] 均匀采样
				stratifiedVotingTimes[v] = early_voters(gen);
			} else {
				// 40% 的节点从 [0.3T, T] 均匀采样
				stratifiedVotingTimes[v] = late_voters(gen);
			}
			// cout<<stratifiedVotingTimes[v]<<endl;
		}
	}
