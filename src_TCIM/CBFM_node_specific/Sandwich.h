...
public:
	...
	static void SandTCIM(InfGraph& g, Argument& arg)
	{
		...
		// g.precomputePoisson(3,10000); // unified setting
		g.precomputeGeometricForAllNodes(1000); // node-specific setting
		...
		double ratio;
		double num_before_t;
		double num_activated_before_vote;
		//compute the objective value
		if(arg.algo=="GB") 
		{
			after_blocking_ADinf = g.MC_based_estimate_with_removal(30000, arg.T, g.seedSet) ;
		}
		else
		{
			after_blocking_ADinf = g.determine_better_solution(arg) ;
			ratio=g.rep_approximation(arg); 
			cout<<"The approximation ratio is : "<<ratio<<endl;
			// for case study
			// num_activated_before_vote= g.MC_based_estimate_with_removal_2(10000, arg.T, g.seedSet);
		...
		of <<initial_ADinf<<"\t"<<after_blocking_ADinf<< "\t"<<(double)interval.count()<< "\t"<< ratio << endl;
		of.close();
	}
};
