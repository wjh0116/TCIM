...
public:
	...
	static void SandTCIM(InfGraph& g, Argument& arg)
	{
		...
		if(arg.algo=="GB") 
		{
			after_blocking_ADinf = g.MC_based_estimate_with_removal(30000, arg.T, g.seedSet) - g.rumorSet.size() * g.U(g.deadline_1);
		}
		else
		{
			after_blocking_ADinf = g.determine_better_solution(arg) - g.rumorSet.size() * g.U(g.deadline_1);
