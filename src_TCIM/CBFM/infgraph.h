...
public:
	...
	/// Mente-Carlo simulations to estimate m/n
	double MC_based_estimate_mn(vector<int> rumorNode, vector<int> Blockers,  int count, int deadline)
	{
		...
		cout<<"estimated edge is:"<<summ/1000.0<<",,, estimated node is: "<<sumn/1000.0<<endl;
		return summ/sumn* 1.0;
	}
	...
	int U(int x) {
		if(x<0) return 0;
		return deadline_1/(double)pow((deadline_1-x+1),A);
	...
	*/
	...
	void generate_sample_TRtree(vector<vector<int>>& sample,vector<vector<int>>& sample_UB,vector<vector<int>>& sample_delay, double &sum, int deadline, vector<int>& AT)
	{
		...
		while (!Q.empty()) 
		{
			...
			I[q] = 1;
			if(!isRumor[q]) sum+=U(deadline_1- AT[q]);
			if(pred[q].size()!=0)
			{
				int pre=pred[q][0];
	...
	double rep_approximation(Argument& arg){
		...
		for(int i=0;i<10000;i++){
			...
			while (!Q.empty()) 
			{
				...
				// cout<<"11"<<endl;
				I[q] = 1;
				if(!isRumor[q]) aft_ulitity+=U(deadline_1- AT[q]);
