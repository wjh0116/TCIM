...
public:    	
    ...
    vector<vector<double>> probT_reverse2; // for reset probability
	vector<vector<double>> probT;
    vector<vector<double>> probT2; // for reset probability
    // vector<vector<int>> delay; // each edge's diffusion delay
    // vector<vector<int>> delay_reverse;
    ...
    void readGraph()
    {
        ...
        for (unsigned int i = 0; i < m; i++)
        {
            ...
            probT_reverse2[b].push_back(p);
            // assuming all the edge's delay is 1
            // delay[a].push_back(1);
            // delay_reverse[b].push_back(1);
        }        
    ...
    {		
        ...
        probT_reverse2 = vector<vector<double>>(n + 2, vector<double>());
		readGraph();
    }
};
