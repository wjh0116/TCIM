...
public:
	...
	struct tl
	{
		...
		int construct_LRRset_stop(int n, int node, vector<int> & isUB, vector<bool>isRumor, vector<vector<int>> sample_reverse)
		{
			auto uStart = node;
	...
	int generate_lrr_stop(vector<int> & isUB){
		...
		vector<int> vis;
		vis.resize(n, 0);
		gen_sample_2(sample, rumorSet, ReachableNode, vis, 0);
		
		int ran = sfmt_genrand_uint32(&sfmtSeed) % Rnode.size();
		int Idx = Rnode[ran];
		if(isUB[Idx]==1) return 1;
		...
		if (vis[Idx] == -1)
		{
			...
			int tmp = tree.construct_LRRset_stop(n, Idx, isUB, isRumor, sample_reverse);
			return tmp;
		}
		return 0;
	}
